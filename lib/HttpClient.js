...
HttpClient.prototype.setTempUrlKey = function(key) {
    this.tempUrlKey = key;
};

HttpClient.prototype.getTempUrlKey = function() {
    return this.tempUrlKey;
};

HttpClient.prototype.setEndpoints = function(response) {
    var body = JSON.parse(response.body);
    var catalog = body.token.catalog;
    var swift = catalog.find(function(e){
        return e.name == 'swift';
    });
    this.endpoints = swift.endpoints;
};

HttpClient.prototype.getEndpointUrl = function(region) {
    var endpoint = this.endpoints.find(function(e){
        return (e.region_id == region) && (e.interface == 'public')
    });
...
HttpClient.prototype.search = function(resourceUrl, nullEncoding) {
    var options = {
        url: resourceUrl,
        method: 'GET',
        headers: {
...
HttpClient.prototype.send = function(options) {
    ...
    this.refreshToken()
        .then(function() {
            ...
            requestOptions.headers['X-Account-Meta-Temp-URL-Key'] = self.tempUrlKey;

            //requestOptions.url = self.getEndpointUrl(self.region).url + requestOptions.url;
