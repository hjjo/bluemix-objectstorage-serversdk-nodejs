    ...
    this.client.search(resourceUrl)
        .then(function(response) {
            ...
            _.forEach(list, function(object) {
                if (object) {
                    var object = new ObjectStorageObject(object.name, self);
                    objectList.push(object);
                }
            });
            deferred.resolve(objectList);
 ...
 *  @return promise
 */
ObjectStorageContainer.prototype.uploadObject = function(req) {
    var deferred = Q.defer();
    var self = this;
    ...
    var resourceUrl = this.baseResourceUrl + '/' + filename;
    ...
    let reqOption = {
            ...
            'X-Auth-Token' : this.client.authToken.token
        },
        qs : {
            'format' : 'json'
        }
    ...
    part.pipe(request(reqOption, (err, res, body) => {
        if(err){
            ...
            var object = new ObjectStorageObject(filename, self);
            deferred.resolve(object);
        }
    }));
    });
