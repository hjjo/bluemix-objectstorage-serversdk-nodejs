...
function ObjectStorage(creds, httpClient) {
    if (creds && creds.projectId && creds.userId && creds.password && creds.region) {
        this.baseResourceUrl =  ((creds.region === 'dallas') ? ObjectStorage.Region.DALLAS : ObjectStorage.Region.LONDON) + creds.projectId;
    ...
    if(creds && creds.tempUrlKey){
        this.client.setTempUrlKey(creds.tempUrlKey);
        this.updateMetadata({
            'Temp-URL-Key' : creds.tempUrlKey
        });
    }
}
