...
function ObjectStorageObject(name, objectStorageContainer) {
    ...
    this.containerName = objectStorageContainer.name;
}
...
ObjectStorageObject.prototype.getTempUrl = function(seconds) {
    var expires = Math.floor(Date.now() / 1000) + seconds;
    var url = this.containerName + '/' + this.name;
    var method = 'GET';
    var key = this.client.getTempUrlKey;
    var objectPath = '/v1/AUTH_' + this.client.authBody.auth.scope.project.id + '/' + url;
    var hmacBody = method + '\n' + expires + '\n' + objectPath;
    var sig = crypto.createHmac('sha1', key).update(hmacBody).digest('hex');
    var tempURL = this.baseResourceUrl + '?temp_url_sig=' + sig + '&temp_url_expires=' + expires;
